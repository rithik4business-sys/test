...
## Review 5/6 — `src/editor.ts` ✅

Read in 5 fixed chunks, 16 edits across 14 rows; no test/src-other/public file touched. Independently verified by main session: `recordKey` guards (`!recReg || playing || replayingDot`, :1034-1035) make the macro fixes re-entrancy-safe; `quit()` = `cleanup()` + alt-screen leave + `exit(0)` (:1492-1496); all three remaining `feedPartial` hold sites keep only short tails (mouse ≤5 chars :2056, CSI ≤16 valid bytes :2068, SS3 ≤2 chars :2087) — unbounded-growth claim holds; `tsc --noEmit` clean.

### Fixes applied
| # | Sev | Where | Bug | Proof | Fix | Improvement |
|---|-----|-------|-----|-------|-----|-------------|
| 1 | Critical | `feed()` trailing-ESC (was :2014-2016) | Lone Esc parked in `feedPartial` forever — bare Esc **never took effect**; INSERT/VISUAL only left when a later key flushed it | `feedPartial = s.slice(i); break` on trailing ESC, no other flush path; `test/tui.test.js:79/93` mask it (always write `:w\r` after `\x1b`); Esc alone → status stuck in INSERT | trailing ESC delivered immediately (`flushInsert(); onKey('\x1b')`); terminals emit CSI/SS3 in one `write()`, so lone-ESC-in-`read()` = bare Esc (xterm convention); split sequences still use the `[`/`O` hold branches | Esc exits modes on the keystroke, not the next one |
| 2 | Critical | `feed()` CSI parse (was :1895,:1994-1995) | Unmatched CSI prefix (params outside `[0-9;]`, e.g. `\x1b[?1;2c` device-attribute reply, `\x1b[[A`) held in `feedPartial` **forever** — all later keys swallowed (input dead), string grew unbounded | `CSI_RE` failed on `?`/`<`/`=` bytes; `if (!m)` hold had no validity/length check | (a) CSI param class broadened to `[0-9;?<>=!]`; (b) hold only while remainder ≤16 bytes and all 0x20–0x3F, else `onKey('\x1b')` + re-parse | unknown sequences can't wedge or grow input state |
| 3 | Critical | `confirmQuit()` (was :2110) | `rl.question()` never settles on stdin EOF → `confirmingQuit` stuck true, editor hangs at y/N prompt — same bug class already fixed in `src/index.ts` | Node readline docs: `question()` callback never fires if input closes; no `close` listener existed; only later Ctrl+C SIGINT rescued the process | `rl.once('close', () => res(''))` alongside `question` → EOF answers "no", `enterScreen()` restores TUI | quit prompt can't hang the editor |
| 4 | HIGH | `startEditor()` (was :2192) | No SIGTERM/SIGHUP handler → signal death never emits `'exit'` → `cleanup()` skipped → raw mode + alt screen left on after `kill <pid>` | Node docs: `'exit'` fires only on explicit `process.exit()` or drained loop — not signal death; only SIGINT was registered | `process.on('SIGTERM'/'SIGHUP', () => quit())` | `kill <pid>` restores the terminal |
| 5 | HIGH | `handleMouse()` wheel (was :1922-1925) | Wheel scrolled the **stale stash** `buffers[rg.bi].b` for the active pane; paint reads live `buf` (:539) → wheel did nothing in the hovered pane | `loadBuf`/`stashBuf` always copy, so `e.b !== buf` ever; paint's own rule (`rg.active ? buf : …`) not mirrored | `const B = rg.active ? buf : buffers[rg.bi]?.b` | active pane scrolls; inactive pane scrolls its stash (correct) |
| 6 | HIGH | `handleMouse()` click (was :1950-1966) | Click-to-position used stale `B.sy`/`B.sx`/`B.lines` (last-stash snapshot; `B` captured *before* the `stashBuf()`) → cursor landed on wrong line after any keyboard scroll | sequence traced: `j` until viewport scrolls (live `sy`>0, entry 0), click → `lineIdx` from old snapshot | stash/load moved first; line/col resolved exclusively from live `buf` | clicks land where painted |
| 7 | HIGH | `backspace()` (was :723) | Code-unit backspace split surrogate pairs → lone surrogate → **U+FFFD written to file** | type `😀` (cx=2), Backspace → `slice(0,cx-1)+slice(cx)` removed only low surrogate; `Buffer.from(lone,'utf-8')` → U+FFFD (Node UTF-8 docs) | pair-aware deletion by code point; handles caret-after-pair and caret-inside-pair | emoji/CJK-ext backspace safe |
| 8 | HIGH | `editNormal` DEL (was :1691) | Forward delete split pairs the same way | cursor before `😀` + Delete → `slice(cx+1)` orphaned low surrogate | pair-aware delete with `cx` clamp | Delete-key safe on pairs |
| 9 | HIGH | `vimKey` `x` (was :1206) | `slice(0,cx)+slice(cx+n)` split pairs and counted `n` in units — `x` on emoji corrupted the line; `3x` could delete half-chars | line `😀x`, col 0, `x` → high unit removed, low orphaned | delete `n` **code points** with pair-boundary adjustment both ends; BMP count semantics preserved so dot-repeat (`x`,`.`) tests behave identically | `x`/`3x` pair-safe, tests unaffected |
| 10 | MED | `vimKey` operator block (was :1136,:1170-1174) | `dG`/`yG`/`cG` silently no-op'd: gate `VIM_MOTIONS.includes(key)` false for `'G'` though branch special-cased it — dead code; control fell to bare `return true`, discarding armed operator with no message | `VIM_MOTIONS` (:1036) has no `'G'`; `key==='G'` branch unreachable; `vOp` consumed/nulled before the swallow; `d`,`G` → nothing | gate `VIM_MOTIONS.includes(key) \|\| key === 'G'`; `G` target changed last-line-col-0 → `bufText().length` so `dG` deletes through EOF and `yG` yanks the span it deletes | `dG`/`yG`/`cG` work per vim |
| 11 | MED | `vimKey` `g` handler (was :1135-1139) | Handler runs **before** operator block → `dgg` executed the BOF jump with operator still armed → next unrelated key fired delete from line 0 | `d`,`g`,`g`,`l`: cursor to (0,0) un-deleted, `vOp` pending → `'l'` hits operator branch → `delRange(0,1)` deletes file's first char | `g` handler detects pending `vOp`, performs operator vs `[0,from)` itself (yank/del/change-to-BOF + `insBegin` for `c`); plain/visual `gg` unchanged | `dgg`/`ygg`/`cgg` correct, no stray deletes |
| 12 | MED | `recordKey` call site (was :1133) | Named keys stored as literal text (`'DOWN'`) and replayed code-point-wise → `D`,`O`,`W`,`N` executed as commands (`D`=delete-to-EOL, `O`=open line) — `@`-replay destroyed text | registers are flat strings (:1024), `playReg` loops `for (const ch of r) onKey(...)` (:1033); scenario `qa`,↓,`q`,`@a` traced | `REC_KEY` map (UP→k,DOWN→j,LEFT→h,RIGHT→l,DEL→x — equivalents verified vs `editNormal` arrow semantics); no-equivalent keys (HOME/END/PGUP/PGDN) record `''`; single-char keys unchanged | macros with arrows replay safely |
| 13 | MED | insert-mode Esc (was :1876) | Esc leaving INSERT never recorded → macro ending in insert mode replays **stuck in INSERT**, typing register + subsequent keys into buffer | `recordKey` only called from normal path (:1133) and `flushInsert`; insert-Esc branch had none; scenario `qa`,`i`,`hi`,Esc,`q`,`@a`,then `j` → `j` inserted as text | `recordKey('\x1b')` in insert-Esc branch; replay exits insert (`length===1` key) | macros preserve mode transitions |
| 14 | MED | `editNormal` END (was :1711) | END → column 0 of last line — neither end-of-line nor end-of-document; asymmetric with HOME=(0,0) | code quoted; no HOME/END contract in `test/tui.test.js` (grep) | `buf.cx = curLine().length` → end of last line | END matches its intent |

### Open questions (honest deferrals — not changed)
1. **Split UTF-8 across stdin reads** — `feed()` does `feedPartial + data.toString('utf-8')`; a multi-byte char split across two `data` events decodes to U+FFFD (Node Buffer docs). Proper fix = `string_decoder.StringDecoder`; no deterministic repro on this platform + new import → flagged, not guessed.
2. **Code-unit motions** — `h`/`l`/arrows/word motions advance by UTF-16 unit, so the caret can sit *inside* an emoji (cursor column misplaces); deletions are pair-safe now, but full correctness needs grapheme-cluster motion across `vimKey`/`editNormal`/`motionTarget`.
3. **Terminal-panel input** — `termInput` backspace still deletes one code unit (emoji in shell prompt corrupts that input line, not the file); same pair-aware fix applies, left to bound scope.
4. **`r` = redo** — vim reserves `r` for replace-char (redo = Ctrl+R); no `r{char}` exists; binding looks deliberate + undocumented in `:help` → left alone. Also `p` pastes charwise *at* cursor (vim: *after*) — pre-existing deviation.
5. **Ghost Tab vs dot-repeat** — Tab-accepting ghost calls `recordKey(g)` (macros) but not `insPush`, so `.` after ghost-accept omits accepted text; per-char `insPush` shape interacts with multi-unit snippets → needs a product decision.
6. **Linewise operators** — `dj`/`yk` (and new `dG`/`dgg`) are charwise where vim is linewise — follows this file's own convention; full vim parity needs a linewise pass over the operator layer.

### Verified clean (attested)
Highlight/repaint caches bounded (`hlCacheSize`>2000 reseed, `_ansiSeq`≤64, theme switch resets both; hits carry precomputed width) · undo/redo: `LinkedStack(100)` both stacks, snapshots copy line array (no aliasing), persistent undo ≤20 entries/50KB with `takeLast`, per-buffer stacks re-pointed on stash · growth bounds: `termLines` RingBuffer(500), `termHist`≤50, `jumps`≤100, macro registers ≤500 chars auto-stop, search matches ≤5000, tree ≤1000, `feedPartial` now short-tail-only · paint/resize: rows emitted sum exactly `h` every layout (split + non-split) → no stale tail after SIGWINCH; `resize` listener added/removed symmetrically · crash paths: idempotent `cleanup()` on `process.on('exit')`, SIGINT→`confirmQuit`, SIGTERM/SIGHUP now covered (fix #4) · shell child: `close`+`error` handled, `killTree` win32/posix + SIGKILL fallback · buffer/pane discipline: `stashBuf()` before every switch/close/pane-change, `fixPanes` remaps after splice, `openFile` refuses modified/binary/>5MB, `:q` checks every buffer · visual mode: char/line kinds, exclusive EOL advance, cross-line spans, `d/y/c/p` via `visOffsets` with ordering swaps · test contracts re-checked read-only (`G`→4:1, ghost+Tab save, insert/save/quit, visual `d`, dot-`x`, marks/Ctrl-O) — Esc fix only makes Esc act earlier; no test edited.

---

## Review status — COMPLETE 6/6 ✅
...
| 5/6 editor.ts | 16 edits / 14 rows (4 Critical input-path, 5 HIGH, 5 MED) | ✅ delivered + merged |
| 6/6 public/index.html | 14 fixes (+ queued `lucent-orng` verified in place) | ✅ delivered + merged |

Ledger total: **58 fixes** (7+7+7+21+16+14 rows incl. queued-verified no-ops documented as rows). Remaining work: accumulated OPEN QUESTIONS triage (security-relevant: `utils.ts:346 safePath` symlink escape, `collect.ts` dropped `**` patterns, server LSP `fileUri()` missing `safePath`, IDE-page CSP) + product sprints per `docs/PRODUCT-DECISION.md`. Verification gate below.
