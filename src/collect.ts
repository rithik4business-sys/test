...
function ruleHits(rule: GitRule, rel: string, isDir: boolean): boolean {
  ...
  if (base.startsWith('*.') && base.indexOf('*', 1) === -1) {
    ...
    if (!name.endsWith(suf) || name === suf.slice(1)) return false;
    return true;
  }
  if (rule.dirOnly) return isDir && (name === base || parts.includes(base));
  return name === base;
}

function gitIgnored(rules: GitRule[], rel: string, isDir: boolean): boolean {
  let hit = false;
  for (const r of rules) {
    if (ruleHits(r, rel, isDir)) hit = !r.neg;
  }
  return hit;
}
 ...
 */
export function collectProjectFiles(dir: string, baseDir?: string, depth = 0): Map<string, string> {
  const files = new Map<string, string>();
  if (depth > 12) return files;
  const base = baseDir || dir;
  const rules = depth === 0 ? parseGitignore(base) : [];
  const collected: Array<{ rel: string; content: string }> = [];
  const walk = (d: string, dep: number): void => {
    if (dep > 12) return;
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        else if (isDot && entry.name !== '.gitignore') continue;
  ...
  for (const c of collected) files.set(c.rel, c.content);
  return files;
}
