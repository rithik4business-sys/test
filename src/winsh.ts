...
export function psCommandArgs(cmd: string): string[] {
  return [
    '-NoProfile',
    '-NonInteractive',
    '-ExecutionPolicy',
...
export function shellFor(platform: string = process.platform): { cmd: string; args: (c: string) => string[] } {
  if (isWindows(platform)) {
    return { cmd: 'powershell.exe', args: (c: string) => psCommandArgs(c) };
  }
  return { cmd: 'sh', args: (c: string) => ['-c', c] };
}
...
export function shellQuote(s: string, platform: string = process.platform): string {
  if (isWindows(platform)) {
...
const POSIX_RUNNERS: Record<string, string> = {
...
const WINDOWS_RUNNERS: Record<string, string> = {
  js: 'node', mjs: 'node', cjs: 'node',
...
export function runnerFor(ext: string, platform: string = process.platform): string | undefined {
