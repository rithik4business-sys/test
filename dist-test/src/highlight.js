...
function isDigitCode(c) { return c >= 48 && c <= 57; }
function isWordStartCode(c) {
    return (c >= 65 && c <= 90) || (c >= 97 && c <= 122) || c === 95 || c === 36 || c === 64;
}
function isWordMidCode(c) {
    return isWordStartCode(c) || isDigitCode(c);
}
function isOperatorCode(c) {
...
function scanNumber(line, i) {
    ...
    if (line[i] === '0' && i + 1 < n) {
        ...
        if (p === 'x' || p === 'X') {
            let j = i + 2;
            while (j < n && isHexCode(line.charCodeAt(j)))
                j++;
            return j > i + 2 ? j : i + 1;
        }
        if (p === 'b' || p === 'B') {
            let j = i + 2;
            while (j < n && (line[j] === '0' || line[j] === '1'))
                j++;
            return j > i + 2 ? j : i + 1;
        }
        if (p === 'o' || p === 'O') {
            let j = i + 2;
            while (j < n && line.charCodeAt(j) >= 48 && line.charCodeAt(j) <= 55)
                j++;
            return j > i + 2 ? j : i + 1;
        }
    }
    let j = i;
    while (j < n && isDigitCode(line.charCodeAt(j)))
        j++;
    ...
    if (j < n && (line[j] === 'e' || line[j] === 'E')) {
        ...
        if (k < n && isDigitCode(line.charCodeAt(k))) {
            k++;
            while (k < n && isDigitCode(line.charCodeAt(k)))
                k++;
            j = k;
        }
    }
...
function tokenizeLine(line, language) {
    ...
    while (i < n) {
        ...
        if (isWordStartCode(c)) {
            let j = i + 1;
            while (j < n && isWordMidCode(line.charCodeAt(j)))
                j++;
        ...
        if (isOperatorCode(c)) {
            let j = i + 1;
            while (j < n && isOperatorCode(line.charCodeAt(j)))
                j++;
...
const _trueFgCache = new Map();
function trueFg(hexColor) {
    ...
    if (_trueFgCache.size > 64)
        _trueFgCache.clear();
    _trueFgCache.set(hexColor, out);
    return out;
}
function colorizeToken(token, palette) {
    if (palette) {
        const hex = token.type === 'keyword' ? palette.keyword :
        ...
        if (hex) {
            ...
            if (seq)
                return `${seq}${token.value}${ANSI.reset}`;
        }
        if (token.type === 'text')
            return token.value;
    }
...
function highlightLine(line, language, palette) {
    if (!line || language === 'text')
        return line;
    const tokens = tokenizeLine(line, language);
    let out = '';
    for (const t of tokens)
        out += colorizeToken(t, palette);
    return out;
}
function highlightCode(code, language, palette) {
