...
exports.listThemeNames = listThemeNames;
...
let themeName = themes_1.DEFAULT_THEME;
try {
    const cfgTheme = (0, utils_1.loadConfig)().theme;
    if (cfgTheme && (0, themes_1.isThemeName)(cfgTheme))
        themeName = cfgTheme;
}
catch { /* keep default */ }
let T = (0, themes_1.getTheme)(themeName);
let cachedPal = {};
function rebuildPal() {
    cachedPal = {
...
function hlEntry(raw, lang) {
    ...
    const h = (0, highlight_1.highlightLine)(raw, lang, cachedPal);
    const entry = { h, w: strWidth(h) };
    if (hlCacheSize > 2000) {
        hlCache.clear();
        hlCacheSize = 0;
        perLang = new Map();
...
function highlightCached(raw, lang) {
    return hlEntry(raw, lang).h;
}
function getThemeName() { return themeName; }
function listThemeNames() { return [...themes_1.THEME_NAMES]; }
function setTheme(name) {
    if (!(0, themes_1.isThemeName)(name))
        return false;
    themeName = name;
    T = (0, themes_1.getTheme)(name);
    rebuildPal();
    hlCache.clear();
    hlCacheSize = 0;
    try {
        ...
        cfg.theme = name;
        (0, utils_1.saveConfig)(cfg);
    }
    catch { /* non-fatal */ }
    try {
        (0, utils_1.scheduleSessionSave)({ theme: name });
    }
    catch { /* non-fatal */ }
    return true;
}
 ...
 * repopulate lazily). */
const _ansiSeq = new Map();
function _seq(kind, h) {
    const key = kind + h;
...
const fg = (h) => _seq(38, h);
const bg = (h) => _seq(48, h);
const RESET = '\x1b[0m';
const REV = '\x1b[7m';
const BOLD = '\x1b[1m';
/** Display width: CJK/emoji count 2, combining marks 0, everything else 1. */
function strWidth(s) {
    const t = (0, highlight_1.stripAnsi)(s);
    let w = 0;
    for (const ch of t) {
...
let buf = { filePath: null, lines: [''], lang: 'text', modified: false, cx: 0, cy: 0, sx: 0, sy: 0 };
let vim = 'normal';
let focus = 'edit';
let cmdBuf = '';
let searchBuf = '';
...
const termLines = new structures_1.RingBuffer(500);
let termInput = '';
let termCursor = 0;
let termHist = [];
let termHistIdx = -1;
...
let undoStack = new structures_1.LinkedStack(100);
let redoStack = new structures_1.LinkedStack(100);
let lastInsertSnapshot = 0;
function say(m, sticky = false) {
    msg = m;
    if (msgTimer) {
...
function snapshotCoalesced(forInsert) {
    const now = Date.now();
...
function undo() {
    ...
    if (!s)
        return say('Nothing to undo', true);
...
function redo() {
    ...
    if (!s)
        return say('Nothing to redo', true);
    undoStack.push({ lines: [...buf.lines], cx: buf.cx, cy: buf.cy });
    buf.lines = s.lines;
    buf.cx = s.cx;
    buf.cy = s.cy;
...
function buildTree() {
    ...
    const walk = (dir, depth) => {
        ...
        const pushList = (list, isDir) => {
            for (const e of list) {
                if (out.length > 1000)
                    return;
                if (e.name.startsWith('.') && e.name !== '.gitignore')
                    continue;
    ...
    tree = out;
    if (out.length > 1000)
        say(`File tree truncated at 1000 entries`, true);
    treeSel = clamp(treeSel, 0, Math.max(0, tree.length - 1));
    treeScroll = clamp(treeScroll, 0, Math.max(0, tree.length - 1));
}
function setLangFor(p) {
    const base = path.basename(p).toLowerCase();
    buf.lang = base === 'dockerfile' ? 'dockerfile' : (0, utils_1.getLanguageFromExt)((0, utils_1.getFileExtension)(p));
}
function resetBufState() {
    buf.cx = 0;
    buf.cy = 0;
    buf.sx = 0;
    buf.sy = 0;
...
function openFile(filePath) {
    ...
    try {
        ...
        if ((0, utils_1.fileExists)(resolved)) {
            if ((0, utils_1.isBinaryFile)(resolved)) {
        ...
        restoreUndo();
        syncEntry();
        say((0, utils_1.fileExists)(resolved) ? `Opened ${path.basename(resolved)}` : `New file ${path.basename(resolved)}`);
        return true;
    }
    catch (err) {
...
function saveFile() {
    if (!buf.filePath) {
        say('No file name — use :w <filename>', true);
        return false;
    }
    try {
        (0, utils_1.writeFileSync)(buf.filePath, buf.lines.join('\n'));
        buf.modified = false;
        persistUndo();
...
function syncTreeScroll(vis) {
    if (treeScroll > treeSel)
        treeScroll = treeSel;
    if (treeSel >= treeScroll + vis)
        treeScroll = treeSel - vis + 1;
}
function syncCodeScroll(vis, ew, B = buf) {
    if (B.cy < B.sy)
        B.sy = B.cy;
    if (B.cy >= B.sy + vis)
        B.sy = B.cy - vis + 1;
...
function ghostFor(B, isActive) {
    if (!isActive || focus !== 'edit') {
        return '';
    }
    ...
    if (key === ghostKey)
        return ghostText;
    ghostKey = key;
    const c = (0, complete_1.suggestCompletion)({ prefix: line, after: '', lines: B.lines, lang: B.lang });
    ghostText = c ? c.text : '';
    return ghostText;
}
/** Append as many ghost chars as fit in the remaining width (width-safe). */
function fitGhost(ghost, used, ew) {
    let w = 0;
    let out = '';
...
function paintCodeRow(y, sy, digits, ew, cw, B = buf, isActive = false, liveSel = true) {
    const idx = sy + (y - 1);
...
function paintTerm(row, termH, w) {
...
function layoutRegions(h, termH) {
...
function termPrint(s) {
    ...
    if (termScroll > 0)
        termScroll = Math.min(termScroll + 1, Math.max(0, termLines.length - 1));
}
function termResolveCd(arg) {
...
function runShell(cmd, cwd) {
    ...
    if (mCd) {
        ...
        if (termHist.length > 50)
            termHist.pop();
        termHistIdx = -1;
        if (dest) {
            termPrevCwd = termCwd;
            termCwd = dest;
            termPrint(`${fg(T.primary)}${shortCwd(termCwd)} ❯${RESET} cd ${mCd[1] ?? ''}`);
        }
        else
            termPrint(fg(T.red) + `cd: no such directory: ${mCd[1] ?? ''}` + RESET);
        paint();
        return;
    }
    ...
    paint();
    const { cmd: sh, args } = (0, winsh_1.shellFor)();
    const workdir = cwd ?? termCwd;
    let p;
    try {
        p = (0, child_process_1.spawn)(sh, args(clean), { cwd: workdir, windowsHide: true });
    }
    catch (e) {
        termBusy = false;
        termPrint(fg(T.red) + `spawn failed: ${e.message}` + RESET);
        paint();
        return;
    }
    termChild = p;
    ...
    p.stdout?.on('data', (d) => onData(d, false));
    p.stderr?.on('data', (d) => onData(d, true));
    const done = (code) => {
        if (termChild === p)
            termChild = null;
        termBusy = false;
...
let visHL = null;
function visualExit() { visual = null; visHL = null; }
function syncVisual() {
    visHL = null;
    if (!visual)
        return;
    ...
    const s = aFirst ? [ay, ax] : [cy, cx];
    const hp = aFirst ? [cy, cx] : [ay, ax];
...
function visRowSpan(idx, len) {
    const h = visHL;
    if (!h || idx < h.y1 || idx > h.y2)
        return null;
    if (h.line)
        return len > 0 ? [0, len] : null;
...
let insEntry = null;
let insChars = [];
function setDot(keys) {
    if (!replayingDot && keys.length)
        dotKeys = [...keys];
}
function insBegin(keys) {
    if (!replayingDot) {
        insEntry = [...keys];
        insChars = [];
    }
}
function insPush(key) {
    if (insEntry && !replayingDot)
        insChars.push(key);
}
function insEnd() {
    if (insEntry && !replayingDot)
        setDot([...insEntry, ...insChars]);
    insEntry = null;
    insChars = [];
}
const localMarks = new Map();
const globalMarks = new Map();
let markWait = false;
let tickWait = null;
let jumps = [];
let jumpIdx = -1;
let lastJump = null;
function markFileKey() { return buf.filePath ?? null; }
function curPos() { return { file: markFileKey(), y: buf.cy, x: buf.cx }; }
function getMark(name) {
    ...
    if (name >= 'A' && name <= 'Z')
        return globalMarks.get(name) ?? null;
    return null;
}
...
function gotoPos(p) {
    ...
    if (p.file !== here) {
        if (buf.modified) {
            say('Unsaved changes — :w first', true);
            return false;
        }
        ...
        if (idx >= 0 && idx !== curIdx()) {
            stashBuf();
            loadBuf(idx);
            panes[activePane] = idx;
        }
        else if (idx < 0) {
            if (!p.file || !(0, utils_1.fileExists)(p.file)) {
                say('Jump target gone', true);
                return false;
            }
            if (!openFile(p.file))
                return false;
...
function jumpBack() {
    if (jumpIdx <= 0) {
        say('At oldest jump', true);
        return;
    }
    lastJump = curPos();
    jumpIdx--;
    if (!gotoPos(jumps[jumpIdx])) {
        jumpIdx++;
        return;
    }
...
function jumpFwd() {
    if (jumpIdx >= jumps.length - 1) {
        say('At newest jump', true);
        return;
    }
    lastJump = curPos();
    jumpIdx++;
...
function bufText() { return buf.lines.join('\n'); }
function offOf(y, x) {
    let o = 0;
    for (let i = 0; i < y && i < buf.lines.length; i++)
        o += buf.lines[i].length + 1;
...
function lineEndExcl(o) {
    const p = posOf(o);
    return offOf(p.y, buf.lines[p.y].length);
}
function motionTarget(kind, from, n) {
    const t = bufText();
    const isW = (c) => /[A-Za-z0-9_]/.test(c || '');
    const fwd = (o) => { while (o < t.length && isW(t[o]))
        o++; while (o < t.length && /\s/.test(t[o]))
        o++; return o; };
    const back = (o) => { while (o > 0 && /\s/.test(t[o - 1]))
        o--; while (o > 0 && isW(t[o - 1]))
        o--; return o; };
    const ls = (o) => offOf(posOfIn(t, o).y, 0);
    const le = (o) => { const p = posOfIn(t, o); return offOf(p.y, buf.lines[p.y].length); };
    ...
    switch (kind) {
        ...
        case 'e': {
            ...
            for (let i = 0; i < n; i++) {
                if (q < t.length)
                    q++;
                while (q < t.length && /\s/.test(t[q]))
                    q++;
                while (q + 1 < t.length && isW(t[q + 1]))
                    q++;
            }
            return q;
        }
...
function objRange(text, o, ch, around) {
    ...
    if (ch === 'w' || ch === 'W') {
        const isW = (c) => /[A-Za-z0-9_]/.test(c || '');
        if (!isW(text[o]) && !around)
            return null;
        let a = o, b = o;
        while (a > 0 && isW(text[a - 1]))
            a--;
        while (b < text.length && isW(text[b]))
            b++;
        if (around)
...
function recordKey(k) {
    if (!recReg || playing || replayingDot)
        return;
    regs[recReg] = (regs[recReg] ?? '') + k;
    if (regs[recReg].length > 500) {
        recReg = null;
        say('Macro too long — stopped', true);
    }
}
function playReg(name, n) {
    const r = regs[name];
    if (r == null) {
        say(`Empty register ${name}`, true);
        return;
    }
    lastReg = name;
    playing = true;
...
function vimKey(key) {
    ...
    if (tickWait) {
        ...
        if (isTick || isName) {
            ...
            if (!target) {
                say(isTick ? 'No previous jump' : `Mark ${key} not set`, true);
                return true;
            }
            lastJump = curPos();
    ...
    if (visual) {
        ...
        if (key === 'v') {
            if (visual.kind === 'char')
                ...
                visual = { kind: 'char', ay: visual.ay, ax: visual.ax };
            return true;
        }
        if (key === 'V') {
            visual = { kind: 'line', ay: visual.ay, ax: visual.ax };
            return true;
        }
        if (key === 'o') {
        ...
        if (key === '.')
            return true; // repeat-over-selection is not recorded; ignore
    ...
    if (vObj) {
        ...
        let range = null;
        if (key === 'w' || key === 'W')
            range = objRange(bufText(), cur, 'w', o.around);
        else if (key.length === 1)
            range = objRange(bufText(), cur, key, o.around);
        if (!range)
            return true;
        ...
        if (o.op === 'y')
            ...
            delRange(range[0], range[1]);
            if (o.op === 'c') {
                vim = 'insert';
                insBegin([o.op, o.around ? 'a' : 'i', key]);
            }
        }
        return true;
    }
    ...
    recordKey(key);
    if (/^[1-9]$/.test(key) || (key === '0' && vCount > 0)) {
        vCount = vCount * 10 + parseInt(key, 10);
        return true;
    }
    if (key === 'g') {
        if (vG) {
    ...
    if (vOp) {
        ...
        if (key === 'i' || key === 'a') {
            vObj = { op: op.op, n: op.n, around: key === 'a' };
            return true;
        }
    ...
    if (VIM_MOTIONS.includes(key)) {
        gotoOff(motionTarget(key, offOf(buf.cy, buf.cx), n));
        return true;
    }
    if (key === 'G') {
...
let activePane = 0;
let splitOn = false;
function curIdx() { return panes[activePane] ?? 0; }
function stashBuf() {
    const e = buffers[curIdx()];
    if (e) {
        e.b = { ...buf, lines: [...buf.lines] };
        e.u = undoStack;
        e.r = redoStack;
    }
}
...
function syncEntry() { stashBuf(); }
function switchBuf(i, force) {
    if (i < 0 || i >= buffers.length) {
        say('No such buffer', true);
        return;
    }
    if (i === curIdx())
        return;
    if (buf.modified && !force) {
        say('Unsaved changes — :w or use ! to force', true);
        return;
    }
    stashBuf();
    persistUndo();
...
function closeBufEntry(i, force) {
    if (i < 0 || i >= buffers.length)
        return;
    stashBuf();
    if (buffers[i].b.modified && !force) {
        say('Unsaved changes — :bd! to force', true);
        return;
    }
    buffers.splice(i, 1);
    if (!buffers.length)
...
function blankEntry() {
    return { b: { filePath: null, lines: [''], lang: 'text', modified: false, cx: 0, cy: 0, sx: 0, sy: 0 }, u: new structures_1.LinkedStack(100), r: new structures_1.LinkedStack(100) };
}
function entryLang(resolved) {
    const base = path.basename(resolved).toLowerCase();
    return base === 'dockerfile' ? 'dockerfile' : (0, utils_1.getLanguageFromExt)((0, utils_1.getFileExtension)(resolved));
}
function readInto(e, filePath) {
    try {
        const resolved = path.resolve(filePath);
        if ((0, utils_1.fileExists)(resolved)) {
            if ((0, utils_1.isBinaryFile)(resolved)) {
            ...
            const data = fs.readFileSync(resolved);
            if (data.length > MAX_OPEN_BYTES) {
                say(`${path.basename(resolved)} is too large to open (${(data.length / 1048576).toFixed(1)}MB)`, true);
                return false;
            }
        ...
        e.u = restoreUndoFor(e.b.filePath);
        e.r.clear();
        return true;
    }
    catch {
...
function fixPanes(removed) {
    panes = panes.map((p) => (p > removed ? p - 1 : p === removed ? -1 : p));
    for (let i = 0; i < panes.length; i++)
        if (panes[i] < 0 || panes[i] >= buffers.length)
            panes[i] = 0;
    if (!panes.length)
        panes = [0];
...
function undoFilePathFor(filePath) {
    try {
...
let lastPersist = 0;
function persistUndoFor(filePath, stack) {
    try {
        if (!filePath)
            return;
...
function restoreUndoFor(filePath) {
    const stack = new structures_1.LinkedStack(100);
    try {
        if (!filePath)
            return stack;
        const p = undoFilePathFor(filePath);
        ...
        if (Array.isArray(u)) {
            for (const e of u.filter((e) => e && Array.isArray(e.lines)).slice(-20))
                stack.push(e);
        }
    }
    catch { /* empty stack */ }
...
function runCmd(raw) {
    ...
    switch (c) {
        ...
        case 'q':
            stashBuf();
            if (buffers.some((e) => e.b.modified))
                say('Unsaved changes — :w or :q! to force', true);
            else
                quit();
            break;
        case 'q!':
        ...
        case 'bd':
        case 'bd!':
            closeBufEntry(panes[activePane], c === 'bd!');
            break;
        case 'sp': {
            stashBuf();
            const e = blankEntry();
            ...
            if (parts[1] && !readInto(e, parts.slice(1).join(' '))) {
                buffers.pop();
                break;
            }
            splitOn = true;
        ...
        case 's': {
            ...
            if (!m) {
                say('Usage: :s/old/new/g', true);
                break;
            }
            if (!m[1]) {
                say('Nothing to replace', true);
                break;
            }
            snapshot();
            let n = 0;
        ...
        case 'rm!': {
            ...
            if (!target) {
                say('Usage: :rm[!] <path>', true);
                break;
            }
            const fp = path.resolve(target);
            try {
                const st = fs.statSync(fp);
                if (st.isDirectory() && c !== 'rm!') {
                    say('Directory — use :rm! to recurse', true);
                    break;
                }
                if (st.isDirectory())
                ...
                buildTree();
                if (buf.filePath === fp)
                    say('Deleted open file — :w recreates it', true);
            }
            catch {
                say('Delete failed — no such file', true);
            }
            break;
        }
        case 'mv': {
            if (parts.length < 3) {
                say('Usage: :mv <old> <new>', true);
                break;
            }
            try {
                const to = path.resolve(parts.slice(2).join(' '));
                if (fs.existsSync(to)) {
                    say('Rename failed — destination already exists', true);
                    break;
                }
                fs.mkdirSync(path.dirname(to), { recursive: true });
                const fromFp = path.resolve(parts[1]);
                fs.renameSync(fromFp, to);
                if (buf.filePath === fromFp) {
                    buf.filePath = to;
                    setLangFor(to);
                }
                buffers.forEach((e) => { if (e.b.filePath === fromFp) {
                    e.b.filePath = to;
                    e.b.lang = entryLang(to);
                } });
                say(`Renamed to ${parts.slice(2).join(' ')}`);
                buildTree();
            }
            catch {
                say('Rename failed', true);
            }
            break;
        }
        case 'run': {
            if (!buf.filePath) {
                say('Open a file first', true);
                break;
            }
            if (buf.modified && !saveFile())
                break;
            const ext = (0, utils_1.getFileExtension)(buf.filePath);
            const runner = (0, winsh_1.runnerFor)(ext);
            if (!runner) {
                say(`No runner for .${ext || '?'}`, true);
                break;
            }
            const dir = path.dirname(buf.filePath);
            runShell(`${runner} ${(0, winsh_1.shellQuote)(path.basename(buf.filePath))}`, dir === '' ? process.cwd() : dir);
            break;
        }
        case 'term': {
            ...
            if (arg === '+' || arg === 'bigger') {
                termMaxed = false;
                termHeight = clamp(termHeight + 2, 6, 30);
            }
            else if (arg === '-' || arg === 'smaller') {
                termMaxed = false;
                termHeight = clamp(termHeight - 2, 6, 30);
            }
            else if (arg === 'max') {
                ...
                termMaxed = false;
                termHeight = clamp(parseInt(arg, 10), 6, 30);
            }
            ...
            termScroll = 0;
            try {
                (0, utils_1.scheduleSessionSave)({ termHeight, termMaxed });
            }
            catch { /* noop */ }
        ...
        case 'tree':
            ...
            try {
                (0, utils_1.scheduleSessionSave)({ treeVisible });
            }
            catch { /* noop */ }
            say(treeVisible ? 'Explorer shown' : 'Explorer hidden');
            break;
        case 'set': {
            ...
            if (key === 'mouse') {
                ...
                say('Usage: :set mouse [on|off]', true);
            break;
        }
        case 'theme':
            if (!parts[1]) {
                say(`theme: ${themeName} — :theme <name> · available: ${themes_1.THEME_NAMES.join(', ')}`, true);
            }
            else {
                ...
                if (setTheme(want)) {
                    ...
                    say(`Unknown theme: ${parts[1]} — available: ${themes_1.THEME_NAMES.join(', ')}`, true);
                }
            }
            break;
        case 'themes':
            say(`themes: ${themes_1.THEME_NAMES.join(', ')}`, true);
            break;
        case 'commands':
            say(`themes: ${themes_1.THEME_NAMES.join(', ')} — :theme <name>`, true);
            break;
        case 'help':
            say(':w :q :e :new :bn/:bp/:ls/:bd · :sp/:only Ctrl+W · :s :rm :mv :run · :theme · :set mouse · :!cmd :term[+/- /max] · term: cd/clear/exit/history/pwd · vim: counts d/c/y/i-a q/@ Ctrl+N · v/V visual o d/y/c · . repeat · ma \'a mA Ctrl+O Tab jumps', true);
            break;
        case '':
            break;
        default:
...
/* ---------- input ---------- */
function visibleMainHeight() {
    const { h } = size();
    const termH = termOpen ? termHeightFor(h) : 0;
    return Math.max(1, h - 1 - termH - 1);
}
function termHeightFor(h) {
    if (termMaxed)
        return clamp(h - 8, 6, h - 4);
...
function cycleFocus() {
    focus = focus === 'edit' ? (treeVisible ? 'tree' : termOpen ? 'term' : 'edit')
        : focus === 'tree' ? (termOpen ? 'term' : 'edit')
            : 'edit';
    if (focus !== 'edit' && vim === 'insert')
        vim = 'normal';
}
function treeActivate() {
...
function wordForward() {
    const l = curLine();
    let x = buf.cx;
    while (x < l.length && isWordChar(l[x]))
        x++;
    while (x < l.length && !isWordChar(l[x]))
        x++;
...
function wordBack() {
    const l = curLine();
    let x = buf.cx;
    while (x > 0 && !isWordChar(l[x - 1]))
        x--;
    while (x > 0 && isWordChar(l[x - 1]))
        x--;
    buf.cx = x;
}
const CSI_MAP = { A: 'UP', B: 'DOWN', C: 'RIGHT', D: 'LEFT', H: 'HOME', F: 'END' };
function editNormal(key) {
    switch (key) {
        case 'i':
            vim = 'insert';
            insBegin(['i']);
        ...
        case 'I':
            ...
            break;
        case 'a':
            buf.cx = Math.min(buf.cx + 1, curLine().length);
            vim = 'insert';
            insBegin(['a']);
            break;
        case 'A':
            buf.cx = curLine().length;
            vim = 'insert';
            insBegin(['A']);
            break;
        case 'o':
        ...
        case '\x15': // Ctrl+U — half page up
            ...
            break;
        case 'UP':
            buf.cy = Math.max(0, buf.cy - 1);
            clampCur();
            break;
        case 'DOWN':
            buf.cy = Math.min(buf.lines.length - 1, buf.cy + 1);
            clampCur();
            break;
        case 'LEFT':
            ...
            break;
        case 'RIGHT':
            buf.cx = Math.min(curLine().length, buf.cx + 1);
            break;
        case 'HOME':
            buf.cy = 0;
            buf.cx = 0;
        ...
        case 'END':
            ...
            break;
        case 'PGUP':
            buf.cy = Math.max(0, buf.cy - visibleMainHeight());
            clampCur();
            break;
        case 'PGDN':
            buf.cy = Math.min(buf.lines.length - 1, buf.cy + visibleMainHeight());
            clampCur();
            break;
    }
}
function killTree(p) {
    ...
    try {
        if (process.platform === 'win32') {
            (0, child_process_1.spawnSync)('taskkill', (0, winsh_1.taskkillArgs)(p.pid), { windowsHide: true, timeout: 5000 });
            return;
        }
        p.kill('SIGTERM');
    }
...
function onKey(key) {
    if (key === '\x03') { // Ctrl+C — kills running term cmd first, never strands output
        ...
        if (buf.modified) {
            say('Unsaved changes — :q! to quit', true);
            paint();
            return;
        }
        quit();
    ...
    if (key === '\x09') {
        ...
        else if (vim === 'normal' && focus === 'edit') {
            jumpFwd();
        } // Tab == Ctrl+I (vim conflation)
        else
            cycleFocus();
        paint();
        return;
    ...
    if (key === '\x07') {
        ...
        paint();
        return;
    } // Ctrl+G — cycle tree/edit/term focus
    if (key === '\x14') { // Ctrl+T
        termOpen = !termOpen;
        focus = termOpen ? 'term' : 'edit';
        paint();
    ...
    if (focus === 'tree') {
        switch (key) {
            ...
            case 'j':
            case 'DOWN':
                treeSel = Math.min(tree.length - 1, treeSel + 1);
                break;
            case 'k':
            case 'UP':
                treeSel = Math.max(0, treeSel - 1);
                break;
            case '\r':
                treeActivate();
                focus = 'edit';
    ...
    if (focus === 'term') {
        ...
        if (key === '\x7f' || key === '\x08') { // backspace at cursor
            if (termCursor > 0) {
                termInput = termInput.slice(0, termCursor - 1) + termInput.slice(termCursor);
                termCursor--;
            }
            paint();
            return;
        ...
        if (key === 'DEL') { // delete at cursor
            if (termCursor < termInput.length)
                termInput = termInput.slice(0, termCursor) + termInput.slice(termCursor + 1);
            paint();
            return;
        }
        if (key === 'LEFT') {
            termCursor = Math.max(0, termCursor - 1);
            paint();
            return;
        }
        if (key === 'RIGHT') {
            termCursor = Math.min(termInput.length, termCursor + 1);
            paint();
            return;
        }
        if (key === 'HOME' || key === '\x01') {
        ...
        if (key === '\x15') {
            termInput = termInput.slice(termCursor);
            termCursor = 0;
            paint();
            return;
        } // Ctrl+U
        if (key === '\x0b') {
            termInput = termInput.slice(0, termCursor);
            paint();
            return;
        } // Ctrl+K
        if (key === '\x17') { // Ctrl+W — delete word back
            let i = termCursor;
            while (i > 0 && termInput[i - 1] === ' ')
                i--;
            while (i > 0 && termInput[i - 1] !== ' ')
                i--;
        ...
        if (key === 'PGUP') {
            termScroll = clamp(termScroll + Math.max(1, termHeightFor(size().h) - 3), 0, Math.max(0, termLines.length - 1));
            paint();
            return;
        }
        if (key === 'PGDN') {
    ...
    if (vim === 'insert') {
        ...
        else if (key === '\x0e' || key === '\x10')
            completeWord(key === '\x0e' ? 1 : -1);
        else if (key === '\r') {
            recordKey('\n');
            insPush('\r');
            newline();
    ...
    if (vim === 'normal' && focus === 'edit' && vimKey(key)) {
        paint();
        return;
    }
    editNormal(key);
...
let mouseOn = false;
try {
    mouseOn = (0, utils_1.loadConfig)().mouse === true;
}
catch { /* keep off */ }
...
function feed(data) {
    ...
    while (i < s.length) {
        if (s[i] === '\x1b' && s[i + 1] === '[' && s[i + 2] === 'M') {
            ...
            flushInsert();
            if (mouseOn)
                handleMouse(s.charCodeAt(i + 3) - 32, s.charCodeAt(i + 4) - 32, s.charCodeAt(i + 5) - 32);
            i += 6;
        }
...
async function confirmQuit() {
    ...
    try {
        ...
        let ans = '';
        try {
            ans = (await new Promise((res) => rl.question(`Unsaved changes in: ${dirty.join(', ')} — quit anyway? (y/N) `, res))).trim().toLowerCase();
        }
        catch {
            ans = '';
        }
...
function startEditor(filePath) {
    ...
    try {
        try {
            const sess = (0, utils_1.loadSession)();
            if (Number.isFinite(sess.termHeight))
                termHeight = Math.max(4, Math.min(60, sess.termHeight));
            if (typeof sess.termMaxed === 'boolean')
                termMaxed = sess.termMaxed;
            if (typeof sess.treeVisible === 'boolean')
                treeVisible = sess.treeVisible;
        }
        catch { /* defaults stand */ }
        buildTree();
        if (!buffers.length)
            buffers.push({ b: { ...buf, lines: [...buf.lines] }, u: undoStack, r: redoStack });
        panes = [0];
        activePane = 0;
        splitOn = false;
        if (filePath) {
            ...
            if ((0, utils_1.fileExists)(resolved)) {
                ...
                const raw = (0, utils_1.readFileSync)(resolved);
        ...
        console.error(`Failed to start editor: ${e.message}`);
        started = false;
        return;
    }
    process.stdout.write('\x1b[?1049h\x1b[?25l');
...
function resetBufStateKeepFile() {
    buf.cx = 0;
    buf.cy = 0;
    buf.sx = 0;
    buf.sy = 0;
...
function getEditorContent() { return buf.lines.join('\n'); }
...
function getCurrentFilePath() { return buf.filePath; }
