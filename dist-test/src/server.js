...
exports.collectProjectFiles = void 0;
exports.findAgentBin = findAgentBin;
...
function findAgentBin(id) {
    const spec = AGENTS[id];
    if (!spec)
        return null;
    try {
        const r = (0, child_process_1.spawnSync)('sh', ['-c', `command -v ${spec.bin}`], { timeout: 5000, encoding: 'utf-8' });
...
function agentStatus() {
    return Object.keys(AGENTS).map((id) => {
        ...
        const st = { id, name: spec.name, installed: bin !== null, run: spec.run, installLabel: spec.installLabel };
        if (bin) {
            st.bin = bin;
            try {
                const r = (0, child_process_1.spawnSync)(bin, spec.versionArgs, { timeout: 10000, encoding: 'utf-8' });
                const v = ((r.stdout || '') + ' ' + (r.stderr || '')).trim().split('\n')[0].trim();
                if (v)
                    st.version = v.slice(0, 80);
            }
...
function startInstall(id) {
    const spec = AGENTS[id];
    if (!spec)
        return { started: false, error: 'unknown agent' };
    if (findAgentBin(id))
        return { started: false, error: 'already installed' };
    if (installJob && installJob.running)
        return { started: false, error: 'another install is already running' };
    const job = { id, running: true, exitCode: null, log: new structures_1.CappedBuffer(60000) };
    installJob = job;
    let child;
    try {
        child = (0, child_process_1.spawn)(spec.installCmd, spec.installArgs, { cwd: os.homedir(), windowsHide: true });
    }
    catch {
        job.running = false;
    ...
    child.on('exit', (code) => { job.running = false; job.exitCode = code ?? 1; push(`\n[exit ${job.exitCode}]\n`); });
...
function ptyAccept(ws, url) {
    if (!ptyMod) {
        try {
            ws.close(1011, 'no pty');
        }
        catch { }
        return;
    }
    if (ptySessions.size >= MAX_PTY) {
        try {
            ws.close(1013, 'busy');
        }
        catch { }
    ...
    let term;
    try {
        term = ptyMod.spawn(shell, [], { name: 'xterm-256color', cols, rows, cwd: ROOT, env: process.env });
    }
    catch {
        try {
    ...
    const id = ++ptySeq;
    const sess = { id, pty: term, ws, shell: shellName };
    ...
    term.onExit(({ exitCode }) => { send({ t: 'exit', code: exitCode ?? 0 }); try {
        ws.close();
    }
    catch { } });
    ws.on('message', (raw) => {
        ...
        if (m.t === 'in' && typeof m.d === 'string') {
            try {
                term.write(m.d.slice(0, 65536));
            }
            catch { }
        }
        else if (m.t === 'resize') {
            try {
                term.resize(clampInt(parseInt(m.cols, 10), 20, 300, cols), clampInt(parseInt(m.rows, 10), 5, 100, rows));
            }
            catch { }
        }
    });
    const dead = () => { ptySessions.delete(id); try {
        term.kill();
    }
    catch { } };
    ws.on('close', dead);
...
function lspSend(s, msg) {
    const body = Buffer.from(JSON.stringify(msg), 'utf-8');
    try {
        s.proc.stdin.write(Buffer.concat([Buffer.from(`Content-Length: ${body.length}\r\n\r\n`, 'utf-8'), body]));
    }
    catch { }
}
function fileUri(p) {
...
function lspAccept(ws, url) {
    ...
    if (!spec) {
        send({ t: 'error', error: 'unsupported language: ' + lang });
        try {
            ws.close();
        }
        catch { }
    ...
    const release = () => { if (!released) {
        released = true;
        activeLsp = Math.max(0, activeLsp - 1);
    } };
    let proc;
    try {
        proc = (0, child_process_1.spawn)(spec.cmd, spec.args, { cwd: ROOT, windowsHide: true });
    }
    catch {
        release();
    ...
    const sess = { proc, buf: '', pending: new Map(), seq: 0, open: new Map(), inited: false, ws };
    let exited = false;
    const fail = () => {
        if (exited)
            return;
    ...
    proc.stdout?.on('data', (d) => {
        sess.buf += String(d);
        if (sess.buf.length > 1048576)
            sess.buf = sess.buf.slice(-524288);
        for (;;) {
            ...
            if (sess.buf.length < start + len)
                break;
            let msg = null;
            try {
                msg = JSON.parse(sess.buf.slice(start, start + len));
            }
            catch { /* fall through */ }
...
function tree(dir, base, depth = 0) {
    if (depth > 6)
        return [];
    let entries;
    try {
    ...
    for (let k = 0; k < 2; k++) {
        ...
        for (const e of list) {
            if (e.name.startsWith('.') && e.name !== '.gitignore')
                continue;
            if (k === 0 && SKIP_DIRS.has(e.name))
                continue;
            const full = path.join(dir, e.name);
            const rel = path.relative(base, full).replace(/\\/g, '/');
            if (k === 0)
                out.push({ name: e.name, path: rel, type: 'dir', children: tree(full, base, depth + 1) });
            else
...
function throttle(req, scope, perMinute) {
    let ip = 'local';
    try {
...
const tokenKindCache = new Map();
const TOKEN_KIND_TTL = 5 * 60 * 1000;
function tokenKindKey(token) {
    return crypto.createHash('sha256').update(token).digest('hex');
}
async function cachedTokenInfo(token) {
    ...
    if (tokenKindCache.size > 500)
        tokenKindCache.clear();
...
function invalidateTokenKind(token) {
    if (!token) {
        tokenKindCache.clear();
        return;
    }
    try {
        tokenKindCache.delete(tokenKindKey(token));
    }
    catch { /* noop */ }
}
function ghError(res, e) {
...
function sanitizePushMessage(m) {
...
function runGit(args, timeoutMs) {
    return new Promise((resolve) => {
        const acc = new structures_1.CappedBuffer(131072);
        let settled = false;
        ...
        let p;
        try {
            p = (0, child_process_1.spawn)('git', args, { cwd: ROOT, windowsHide: true });
        }
        catch (e) {
            done(`spawn failed: ${e.message}`, 127);
            return;
        }
        runningExecs.add(p);
        ...
        p.stdout?.on('data', (d) => { acc.push(String(d)); });
        p.stderr?.on('data', (d) => { acc.push(String(d)); });
        p.on('error', (e) => { runningExecs.delete(p); clearTimeout(kill); done(`spawn failed: ${e.message}`, 127); });
        p.on('close', (code) => {
...
function binCache(key, load) {
    const hit = _binCache.get(key);
    if (hit)
        return hit;
    const data = load();
    if (_binCache.size > 32)
        _binCache.clear();
...
function idePage() {
    ...
    for (const p of candidates) {
        try {
            ...
            if (_cachedPage !== null && _cachedPagePath === p && mtime === _cachedPageMtime)
                return _cachedPage;
...
function cachedTreeJSON() {
    const now = Date.now();
    if (_treeCache && (now - _treeCache.ts) < TREE_CACHE_TTL)
        return _treeCache.json;
    const data = tree(ROOT, ROOT);
    const json = JSON.stringify(data);
    _treeCache = { data, json, ts: now };
    return json;
}
function invalidateTreeCache() { _treeCache = null; }
...
function execFileTimeout(cmd, args, opts = {}) {
    return new Promise((resolve) => {
        let p;
        try {
            p = (0, child_process_1.spawn)(cmd, args, { windowsHide: true, env: { ...process.env, ...(opts.env || {}) } });
        }
        catch (e) {
            resolve({ stdout: '', stderr: String(e.message), exit: 127 });
            return;
        }
        runningExecs.add(p);
        let out = '';
        ...
        catch { /* noop */ } }, opts.timeoutMs || 10000);
        p.stdout?.on('data', (d) => { out += String(d); if (out.length > 65536)
            out = out.slice(-65536); });
        p.stderr?.on('data', (d) => { err += String(d); if (err.length > 8192)
            err = err.slice(-8192); });
        try {
            if (opts.input !== undefined && p.stdin)
                p.stdin.write(opts.input);
            if (p.stdin)
                p.stdin.end();
        }
        catch { /* noop */ }
        p.on('error', (e) => { runningExecs.delete(p); clearTimeout(to); resolve({ stdout: out, stderr: String(e.message), exit: 127 }); });
        p.on('close', (code) => { runningExecs.delete(p); clearTimeout(to); resolve({ stdout: out, stderr: err, exit: code ?? 0 }); });
    });
}
function startServer(port = 3000, host = '127.0.0.1') {
    return new Promise((resolve, rejectPromise) => {
        const server = http.createServer(async (req, res) => {
            try {
                ...
                if (url.pathname === '/healthz' && req.method === 'GET') {
                    ...
                    res.end(JSON.stringify({ ok: true, version: (0, utils_1.getVersion)() }));
                    return;
                }
                    ...
                    try {
                        const data = binCache('icon:' + name, () => {
                            ...
                            for (const base of bases) {
                                try {
                                    return fs.readFileSync(path.join(base, name));
                                }
                                catch { /* next */ }
                            }
                ...
                if (url.pathname === '/api/file' && req.method === 'GET') {
                    ...
                    const fp = (0, utils_1.safePath)(ROOT, rel);
                    if (!fp || (0, utils_1.isHiddenRel)(relPosix(fp))) {
                        res.writeHead(403, { 'Content-Type': 'application/json' });
                ...
                if (url.pathname === '/api/save' && req.method === 'POST') {
                    ...
                    const fp = (0, utils_1.safePath)(ROOT, String(p));
                    if (!fp || (0, utils_1.isHiddenRel)(relPosix(fp))) {
                        res.writeHead(403, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ error: 'forbidden' }));
                        return;
                ...
                if (url.pathname === '/api/exec' && req.method === 'POST') {
                    ...
                    let workdir = ROOT;
                    if (cwd) {
                        const dir = (0, utils_1.safePath)(ROOT, cwd);
                        if (!dir) {
                            res.writeHead(403, { 'Content-Type': 'application/json' });
                            res.end(JSON.stringify({ error: 'forbidden' }));
                            return;
                    ...
                    try {
                        const result = await new Promise((resolvePromise) => {
                            const acc = new structures_1.CappedBuffer(65536);
                            const push = (d) => { acc.push(String(d).replace(/\r/g, '')); };
                            const p = (0, child_process_1.spawn)(shell, args, { cwd: workdir, windowsHide: true });
                            runningExecs.add(p);
                            const drop = () => { runningExecs.delete(p); };
                            p.on('error', (e) => { drop(); clearTimeout(kill); resolvePromise({ output: `spawn failed: ${e.message}`, exit: 127 }); });
                            const kill = setTimeout(() => {
                                try {
                                    p.kill('SIGTERM');
                                }
                ...
                if (url.pathname === '/api/delete' && req.method === 'POST') {
                    ...
                    let p = '';
                    try {
                        p = String(JSON.parse(body || '{}').p || '');
                    }
                    catch { /* bad json */ }
                    const fp = p ? (0, utils_1.safePath)(ROOT, p) : null;
                    if (!fp || fp === ROOT || (0, utils_1.isHiddenRel)(relPosix(fp))) {
                ...
                if (url.pathname === '/api/rename' && req.method === 'POST') {
                    ...
                    const fromFp = from ? (0, utils_1.safePath)(ROOT, from) : null;
                    const toFp = to ? (0, utils_1.safePath)(ROOT, to) : null;
                    if (!fromFp || !toFp || fromFp === ROOT || toFp === ROOT) {
                        res.writeHead(400, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ error: 'bad request' }));
                        return;
                    }
                    if ((0, utils_1.isHiddenRel)(relPosix(fromFp)) || (0, utils_1.isHiddenRel)(relPosix(toFp))) {
                        res.writeHead(403, { 'Content-Type': 'application/json' });
                        res.end(JSON.stringify({ error: 'forbidden' }));
                        return;
                    }
                ...
                if (url.pathname === '/api/search' && req.method === 'POST') {
                    ...
                    if (!q || q.length > 200) {
                        sendJson(res, 400, { error: 'query required (max 200 chars)' });
                        return;
                    }
                    let re = null;
                    try {
                    ...
                    const walk = (dir, depth) => {
                        if (out.length >= 100 || filesSeen >= 2000 || depth > 8)
                            return;
                        let entries;
                        try {
                            entries = fs.readdirSync(dir, { withFileTypes: true });
                        ...
                        for (const e of entries) {
                            if (out.length >= 100 || filesSeen >= 2000)
                                return;
                            if (e.isSymbolicLink())
                                continue;
                            const full = path.join(dir, e.name);
                            ...
                            if (e.isDirectory()) {
                                ...
                                for (let i = 0; i < lines.length && out.length < 100; i++) {
                                    ...
                                    if (hit)
                                        out.push({ p: rel, line: i + 1, text: lines[i].slice(0, 200) });
                                }
                            }
                        }
                    };
                ...
                if ((url.pathname === '/api/git/status' || url.pathname === '/api/git/diff') && req.method === 'GET') {
                    ...
                    if (r.exit !== 0) {
                        sendJson(res, 502, { error: 'git failed' });
                        return;
                    }
                    if (!isDiff) {
                        ...
                        for (const ln of lines) {
                            ...
                            if (ln.length > 3)
                                files.push({ x: ln[0], y: ln[1], p: ln.slice(3).replace(/^"(.+)"$/, '$1') });
                        }
                ...
                if ((url.pathname === '/api/git/commit' || url.pathname === '/api/git/pull' || url.pathname === '/api/git/init') && req.method === 'POST') {
                    ...
                    if (url.pathname === '/api/git/init') {
                        ...
                        if (r.exit !== 0) {
                            sendJson(res, 502, { error: 'git init failed' });
                            return;
                        }
                        invalidateTreeCache();
                    ...
                    if (url.pathname === '/api/git/pull') {
                        ...
                        if (r.exit !== 0) {
                            sendJson(res, 502, { error: r.output.slice(-300) || 'git pull failed' });
                            return;
                        }
                        invalidateTreeCache();
                    ...
                    if (!message) {
                        sendJson(res, 400, { error: 'commit message required' });
                        return;
                    }
                    const add = await runGit(['add', '-A'], 30000);
                    if (add.exit !== 0) {
                        sendJson(res, 502, { error: 'git add failed' });
                        return;
                    }
                    const commit = await runGit(['commit', '-m', message], 30000);
                    if (commit.exit !== 0) {
                        sendJson(res, 502, { error: commit.output.slice(-300) || 'git commit failed' });
                        return;
                    }
                    invalidateTreeCache();
                ...
                if (url.pathname === '/api/github/status' && req.method === 'GET') {
                    ...
                    const token = ghToken();
                    if (!token) {
                        sendJson(res, 200, { connected: false, username: null, selectedRepo: (0, utils_1.getSelectedRepo)() });
                        return;
                    }
                    try {
                        const info = await cachedTokenInfo(token);
                        if (info.login && info.login !== (0, utils_1.loadConfig)().githubUsername) {
                            try {
                                (0, utils_1.setGitHubUsername)(info.login);
                            }
                            catch { /* noop */ }
                        }
                        sendJson(res, 200, { connected: true, username: info.login, name: info.name || null, avatar: info.avatar || null, selectedRepo: (0, utils_1.getSelectedRepo)(), tokenKind: info.kind, appLogin: gh.isGitHubAppToken(token) });
                    }
                    catch (e) {
                        if (e?.statusCode === 401) {
                            try {
                        ...
                        sendJson(res, 200, { connected: false, username: null, selectedRepo: (0, utils_1.getSelectedRepo)(), stale: true });
                    }
                    return;
                }
                ...
                if (url.pathname === '/api/github/device/poll' && req.method === 'POST') {
                    ...
                    if (devicePolls.size > 200)
                        devicePolls.clear();
                    try {
                        const r = await gh.pollDeviceOnce(dc);
                        if (r.status === 'authorized') {
                            invalidateTokenKind();
                            (0, utils_1.saveToken)(r.token);
                            ...
                            const kind = gh.classifyTokenKind(checked.scopes);
                            (0, utils_1.setGitHubUsername)(checked.login);
                ...
                if (url.pathname === '/api/github/token' && req.method === 'POST') {
                    ...
                    try {
                        ...
                        if (guard) {
                            sendJson(res, 403, { error: guard });
                            return;
                        }
                        (0, utils_1.saveToken)(t);
                        (0, utils_1.setGitHubUsername)(checked.login);
                ...
                if (url.pathname === '/api/github/cli' && req.method === 'POST') {
                    ...
                    try {
                        ...
                        if (g.exit === 127 && /ENOENT|not found/i.test(g.stderr))
                            ghMissing = true;
                    ...
                    for (const cand of candidates) {
                        try {
                            ...
                            if (guard) {
                                lastErr = cand.source + ': ' + guard;
                                continue;
                            }
                            (0, utils_1.saveToken)(cand.token);
                            (0, utils_1.setGitHubUsername)(checked.login);
                            const kind = gh.classifyTokenKind(checked.scopes);
                            ...
                            lastErr = cand.source + ': ' + ((e?.statusCode === 401) ? 'stored login expired' : 'validation failed');
                        }
                    }
                ...
                if (url.pathname === '/api/github/logout' && req.method === 'POST') {
                    if (!needAuth(req, url, res))
                        return;
                    try {
                        invalidateTokenKind(ghToken() || undefined);
                    }
                    catch { /* noop */ }
                    try {
                        (0, utils_1.clearToken)();
                ...
                if (url.pathname === '/api/github/repos' && req.method === 'POST') {
                    ...
                    if (!gh.isValidRepoName(name)) {
                        sendJson(res, 400, { error: 'invalid repo name (A-Z a-z 0-9 . _ -)' });
                        return;
                    }
                    try {
                        ...
                        try {
                            ...
                            if (blocked) {
                                sendJson(res, 403, { error: blocked, tokenKind: kind, appLogin: gh.isGitHubAppToken(token) });
                                return;
                            }
                        }
                        catch { /* scope probe failed — fall through to the real call */ }
                        const repo = await gh.createRepo(token, name, description, isPrivate);
                        try {
                            (0, utils_1.setSelectedRepo)(repo.full_name);
                        }
                        catch { /* noop */ }
                        ...
                        if (friendly) {
                            sendJson(res, 403, { error: friendly });
                            return;
                        }
                        ghError(res, e);
                    }
                ...
                if (url.pathname === '/api/github/repo' && req.method === 'GET') {
                    ...
                    if (!parsed) {
                        sendJson(res, 400, { error: 'bad repo (owner/name)' });
                        return;
                    }
                    try {
                        const [r, topics] = await Promise.all([
                            gh.getRepoFull(token, parsed.owner, parsed.repo),
                            gh.getTopics(token, parsed.owner, parsed.repo).catch(() => []),
                        ]);
                        sendJson(res, 200, {
                            name: r.name, full_name: r.full_name, html_url: r.html_url, private: !!r.private,
                ...
                if (url.pathname === '/api/github/repo' && req.method === 'PATCH') {
                    ...
                    try {
                        ...
                        if (Object.keys(patch).length)
                            r = await gh.updateRepo(token, parsed.owner, parsed.repo, patch);
                        let topics = null;
                        if (topicsRaw !== undefined) {
                            const clean = gh.sanitizeTopics(topicsRaw);
                            if (clean === null) {
                                sendJson(res, 400, { error: 'invalid topics' });
                                return;
                            }
                            topics = await gh.setTopics(token, parsed.owner, parsed.repo, clean);
                        }
                        if (!r)
                            r = await gh.getRepoFull(token, parsed.owner, parsed.repo);
                        if (patch.name && r.full_name) {
                            try {
                                (0, utils_1.setSelectedRepo)(r.full_name);
                            }
                            catch { /* noop */ }
                        }
                        sendJson(res, 200, {
                ...
                if (url.pathname === '/api/github/select' && req.method === 'POST') {
                    ...
                    try {
                        ...
                        (0, utils_1.setSelectedRepo)(`${parsed.owner}/${parsed.repo}`);
                ...
                if (url.pathname === '/api/github/push' && req.method === 'POST') {
                    ...
                    const fullRaw = typeof body.full === 'string' && body.full.trim() ? body.full.trim() : ((0, utils_1.getSelectedRepo)() || '');
                    const parsed = gh.parseRepoFull(fullRaw);
                    if (!parsed) {
                        sendJson(res, 400, { error: 'no repo selected' });
                        return;
                    }
                    try {
                        const files = (0, collect_1.collectProjectFiles)(ROOT);
                        if (files.size === 0) {
                            sendJson(res, 400, { error: 'no files to push' });
                            return;
                        }
        ...
        if (WsMod && ptyMod) {
            ...
            server.on('upgrade', (req, socket, head) => {
                ...
                if (url.pathname === '/api/pty') {
                    ...
                    wssPty.handleUpgrade(req, socket, head, (ws) => ptyAccept(ws, url));
                }
                    ...
                    wssLsp.handleUpgrade(req, socket, head, (ws) => lspAccept(ws, url));
                }
                else {
                    try {
                        socket.destroy();
        ...
        server.listen(port, host, () => {
            ...
            if (!AUTH_TOKEN)
                console.log(`  ⚠  exec endpoint disabled without TYPEWRITER_TOKEN`);
            if (host === '0.0.0.0')
                console.log(`  ⚠  listening on all interfaces — set TYPEWRITER_TOKEN`);
            console.log('');
            resolve();
        });
    });
